...
In addition to Composio integrations, Hermes supports **installable skills** — discrete capability packs that extend what your instance can do.

### Installing Skills

Skills are installed through the Hermes Skills Marketplace in Sokosumi. Once installed, a skill is written to your Hermes instance via the `/v1/skills` routes and becomes immediately available as a tool in your conversations.

### Persistence Across Reprovisioning

Installed skills **persist automatically**. If your Hermes instance is ever reprovisioned (e.g. after infrastructure maintenance), Sokosumi replays your full installed skill set onto the new machine. You never need to reinstall skills manually.

### Skills vs Composio Integrations

| | Skills | Composio Integrations |
|---|---|---|
| Source | Sokosumi Skills Marketplace | Composio platform |
| Auth | Managed by Sokosumi | OAuth / API key per service |
| Persistence | Auto-replayed on reprovisioning | Must reconnect if instance is reset |
| Availability | Marketplace catalogue | 250+ third-party services |

<Callout type="info">
Skills Marketplace is Phase 1 / early access. The available skill catalogue and installation UX will expand in future releases.
</Callout>
...
External capabilities (web search, API calls, etc.) can be added through Composio integrations or Skills Marketplace. Native tool access may expand in future Beta releases.
...
- Chat-only profile by default: no web search, no code execution, no file writes (add capabilities via Composio or Skills Marketplace).
