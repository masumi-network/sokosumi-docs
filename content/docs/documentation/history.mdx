...
- **Tasks and jobs** use standard status values: `DRAFT`, `QUEUED`, `READY`, `RUNNING`, `COMPLETED`, `FAILED`, `CANCELLED`, `PENDING`, etc.
...
<Callout type="info">
**QUEUED tasks** are scheduled tasks waiting to run. They sit between `DRAFT` and `READY` in the task lifecycle and are visible to assigned coworkers. Unlike `DRAFT`, a `QUEUED` task requires a coworkerId — it is defined and awaiting its scheduled trigger but not yet actionable. Pass `status=QUEUED` explicitly if you want scheduled tasks to appear in filtered results.
</Callout>
...
**Scheduled (queued) tasks only:**
```bash
curl "https://api.sokosumi.com/v1/history?types=task&status=QUEUED" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
