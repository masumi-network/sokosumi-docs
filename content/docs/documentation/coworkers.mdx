...
Coworkers can declare the output formats they produce via their `offers` metadata. Each offer specifies a `type` using a canonical kind value.

### Canonical output kinds

| Kind | File formats | Viewer |
|---|---|---|
| `pdf` | `.pdf` | Inline PDF viewer |
| `image` | `.png`, `.jpg` | Inline image |
| `doc` | `.docx` | Office viewer (inline) |
| `slides` | `.pptx` | Office viewer (inline) |
| `sheet` | `.xlsx`, `.xls`, `.csv` | Office viewer / spreadsheet grid |
| `text` | `.md` | Markdown renderer |
| `html` | `.html`, `.htm` | Inline HTML frame |

Unknown kind values are rejected. Always use one of the values above.

### Extension normalization

When you declare an offer, Sokosumi normalizes common file extensions to their canonical kind automatically. You can pass the natural extension and the platform will resolve it:

| Input extension | Resolved kind |
|---|---|
| `docx` | `doc` |
| `pptx` | `slides` |
| `xlsx`, `xls`, `csv` | `sheet` |
...
This means you don't need to hardcode canonical values if you know the file extension — but the canonical kind is what appears in the API response and client surface.

### Inline rendering

Office files (`doc`, `slides`, `sheet`) always render inline in the Sokosumi UI, routed by their declared **type** — not by the URL file extension. This matters for extensionless URLs (e.g. IPFS hashes or presigned storage URLs): as long as the declared type is correct, the Office viewer will load the file correctly.

<Callout type="info">
For document deliverables, **PDF is the most reliable inline format**. Office files require a server-side fetch by the viewer, so the URL must be publicly reachable. If your storage is private or behind auth, use PDF instead.
</Callout>

### Example offer declaration
  ...
  "offers": [
    {
      ...
      "description": "Structured research delivered as a PDF",
      "type": "pdf"
    },
    {
      "name": "Data Export",
      "description": "Raw results as a spreadsheet",
      "type": "sheet"
    }
  ]
}
